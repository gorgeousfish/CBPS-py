...
``cbps.fit_multiple()``  Batch estimation across multiple datasets
``cbps.set_verbosity()`` Control package logging verbosity
======================== ================================================
...
The :func:`~cbps.diagnostics.weights_diag.weight_diagnostics` function provides
comprehensive assessment of IPW weight quality:
...
.. code-block:: python
    ...
    print(f"Effective sample size: {diag['ess']:.0f} / {len(df)}")
    print(f"ESS ratio: {diag['ess_ratio']:.3f}")
    print(f"Coefficient of variation: {diag['cv']:.3f}")
    print(f"Extreme weights (>10x median): {diag['n_extreme']}")

Overlap and Normality Diagnostics
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Check positivity assumptions and treatment model diagnostics:
...
.. code-block:: python
    ...
    # Overlap check (binary treatment)
    overlap = check_overlap(fit.fitted_values, df['treat'].values)
    if overlap['violation_detected']:
        print(f"WARNING: Overlap violation detected")
        print(f"Consider trimming at alpha={overlap['recommended_alpha']}")

    # Normality check (continuous treatment)
    import numpy as np
    X = np.column_stack([np.ones(100), np.random.randn(100, 3)])
    T_cont = X @ [1, 0.5, -0.3, 0.2] + np.random.randn(100)
    norm_test = test_treatment_normality(T_cont, X)
    print(f"Normality p-value: {norm_test['p_value']:.4f}")

Logging Verbosity Control
^^^^^^^^^^^^^^^^^^^^^^^^^

Control diagnostic output during estimation:

.. code-block:: python

    from cbps import set_verbosity

    # Silent mode (default) - only warnings
    set_verbosity(0)

    # Progress messages - optimization steps
    set_verbosity(1)

    # Full debug output - detailed numerical diagnostics
    set_verbosity(2)

Batch Estimation with fit_multiple
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Fit CBPS across multiple datasets (useful for simulations):
...
.. code-block:: python
    ...
    # Generate multiple simulation datasets
    datasets = []
    for _ in range(100):
        n = 500
        X = np.random.randn(n, 3)
        ps = 1 / (1 + np.exp(-(X @ [0.5, -0.3, 0.2])))
        treat = np.random.binomial(1, ps)
        df = pd.DataFrame({'treat': treat, 'x1': X[:,0], 'x2': X[:,1], 'x3': X[:,2]})
        datasets.append(df)

    # Batch estimation
    results = cbps.fit_multiple('treat ~ x1 + x2 + x3', datasets, att=0)
    successful = [r for r in results if hasattr(r, 'coefficients')]
    print(f"Converged: {len(successful)} / {len(datasets)}")

WeightNormalizer
^^^^^^^^^^^^^^^^

The :class:`~cbps.utils.weights.WeightNormalizer` class provides unified
Hajek-style weight standardization:

.. code-block:: python

    from cbps.utils.weights import WeightNormalizer
    import numpy as np

    # ATE normalization (group sums to 1)
    weights = np.array([2.0, 1.5, 0.5, 0.8, 1.2, 0.3])
    treat = np.array([1, 1, 1, 0, 0, 0])
    normalized = WeightNormalizer.normalize_ate(weights, treat)

    # ATT normalization (requires propensity scores)
    probs = np.array([0.6, 0.5, 0.4, 0.3, 0.4, 0.2])
    normalized_att = WeightNormalizer.normalize_att(weights, treat, probs)
